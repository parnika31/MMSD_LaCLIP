...
def train(args, model, device, train_data, dev_data, test_data, processor):
    ...
    for i_epoch in trange(0, int(args.num_train_epochs), desc="Epoch", disable=False):
        ...
        print('train_loss :', sum_loss/sum_step)
        ...
        if dev_acc > max_acc:
            ...
            torch.save(model_to_save.state_dict(), os.path.join(path_to_save, 'model_laion_clip_test_run.pt')) # model_laion_LAclip_test_run
...
def evaluate_acc_f1(args, model, device, data, processor, macro=False,pre = None, mode='test'):
        ...
        if mode == 'test':
            ...
            print('test_loss:', sum_loss/sum_step)
        else:
            wandb.log({'dev_loss': sum_loss/sum_step})
            print('dev_loss:', sum_loss/sum_step)
        if pre != None:
